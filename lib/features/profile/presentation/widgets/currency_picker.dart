...
const List<({String code, String name})> kSupportedCurrencies =
...
class CurrencyPickerField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ({String code, String name}) current = kSupportedCurrencies
        .firstWhere(
    ...
    return InkWell(
      ...
      onTap: () async {
        final String? picked = await AppFeedback.sheet<String>(
          context,
          child: _CurrencySheet(selected: selected),
        );
      ...
      child: Container(
        ...
        child: Row(
          children: <Widget>[
            Text(
              NumX.currencySymbolFor(current.code),
...
class _CurrencySheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AppBottomSheet(
      ...
      child: Column(
        children: <Widget>[
          for (final ({String code, String name}) currency
              in kSupportedCurrencies)
            ListTile(
              leading: SizedBox(
                width: 36,
                child: Text(
                  NumX.currencySymbolFor(currency.code),
                  style: context.text.titleMedium,
                ),
              ),
