...
void main() {
  ...
  group('create', () {
    test('persists a row and marks it pending create', () async {
      final Result<Transaction> result = await repository.create(
        make(id: 'tx-1'),
      );
    ...
    test('rejects a transfer with no destination', () async {
      final Result<Transaction> result = await repository.create(
        make(id: 'tx-1', type: TransactionType.transfer),
      );
  ...
  group('update', () {
    test('keeps pendingCreate for a row the server has never seen', () async {
      ...
      final Result<Transaction> updated = await repository.update(
        make(id: 'tx-1', amount: 99),
      );
  ...
  group('delete and restore', () {
    ...
    test('tombstones a synced row so the deletion can be replayed', () async {
      ...
      final List<Transaction> pending =
          (await repository.pendingSync()).dataOrNull!;
  ...
  group('queries', () {
    ...
    test('filters by category', () async {
      final Result<TransactionPage> page = await repository.getPage(
        const TransactionQuery(categoryIds: <String>{'cat-food'}),
      );
    ...
    test('filters by date range inclusively', () async {
      ...
      expect(
        page.dataOrNull?.items.map((Transaction t) => t.id).toSet(),
        <String>{'a', 'b'},
      );
    });
    ...
    test('filters by amount range', () async {
      final Result<TransactionPage> page = await repository.getPage(
        const TransactionQuery(minAmount: 20, maxAmount: 500),
      );
      expect(
        page.dataOrNull?.items.map((Transaction t) => t.id).toSet(),
        <String>{'b', 'c'},
    ...
    test('searches notes case-insensitively', () async {
      final Result<TransactionPage> page = await repository.getPage(
        const TransactionQuery(search: 'LUNCH'),
      );
      expect(page.dataOrNull?.items.single.id, 'b');
    });
    ...
    test('searches tags', () async {
      final Result<TransactionPage> page = await repository.getPage(
        const TransactionQuery(search: 'work'),
      );
      expect(page.dataOrNull?.items.single.id, 'b');
    });
    ...
    test('combines filters with AND semantics', () async {
      final Result<TransactionPage> page = await repository.getPage(
        TransactionQuery(
          types: const <TransactionType>{TransactionType.expense},
          from: DateTime(2026, 8),
          to: DateTime(2026, 8, 31),
        ),
      );
    ...
    test('sorts by date descending by default', () async {
      ...
      expect(
        page.dataOrNull?.items.map((Transaction t) => t.id).toList(),
        <String>['b', 'a', 'c'],
      );
    });
    ...
    test('sorts by amount', () async {
      final Result<TransactionPage> page = await repository.getPage(
        const TransactionQuery(sort: TransactionSort.amountDesc),
      );
      expect(
        page.dataOrNull?.items.map((Transaction t) => t.id).toList(),
        <String>['a', 'c', 'b'],
    ...
    test('paginates cumulatively and reports hasMore', () async {
      final Result<TransactionPage> first = await repository.getPage(
        const TransactionQuery(pageSize: 2),
      );
      expect(first.dataOrNull?.items, hasLength(2));
      expect(first.dataOrNull?.hasMore, isTrue);
      expect(first.dataOrNull?.total, 3);

      final Result<TransactionPage> second = await repository.getPage(
        const TransactionQuery(pageSize: 2, page: 1),
      );
  ...
  group('aggregates', () {
    setUp(() async {
      ...
      await repository.create(
        make(id: 'b', amount: 250, date: DateTime(2026, 8, 2)),
      );
      await repository.create(
        make(
          id: 'c',
    ...
    test('summary excludes transfers from the balance', () async {
      final TransactionSummary summary = await repository
          .watchSummary(from: DateTime(2026, 8), to: DateTime(2026, 8, 31))
          .first;
    ...
    test('savings rate is 0 rather than NaN with no income', () async {
      final TransactionSummary summary = await repository
          .watchSummary(from: DateTime(2026, 9), to: DateTime(2026, 9, 30))
          .first;
    ...
    test('totalsByCategory ranks descending', () async {
      ...
      final Result<Map<String, double>> totals = await repository
          .totalsByCategory(
            from: DateTime(2026, 8),
            to: DateTime(2026, 8, 31),
            expensesOnly: true,
    ...
    test('dailyTotals zero-fills days with no activity', () async {
      final Result<List<({DateTime day, double income, double expense})>>
      series = await repository.dailyTotals(
        from: DateTime(2026, 8),
  ...
  group('recurring', () {
    test('materialises occurrences that fell due while the app was closed', () async {
      final DateTime twoMonthsAgo = DateTime.now().subtract(
  ...
  group('export and import', () {
    test('round-trips through export/import', () async {
      await repository.create(
        make(id: 'tx-1', note: 'Coffee', tags: const <String>['cafe']),
      );

      final List<Transaction> exported =
          (await repository.exportAll()).dataOrNull!;
      ...
      final Transaction restored =
          (await repository.getById('tx-1')).dataOrNull!;
  ...
  group('watchPage', () {
    test('re-emits when a transaction is written', () async {
      final Stream<TransactionPage> stream = repository.watchPage(
        const TransactionQuery(),
      );
